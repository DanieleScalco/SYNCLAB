...
declare const rxjs: any; // Per non dare errori su rxjs gli diamo tipo any
...
function getBooks(bookTitle: string) {
	const { from } = rxjs;	// Anche se typescript non riconosce l'oggetto rxjs l'html usando il .js prodotto lo vedrà
...
function showTotal(totalItems: number) {
	...
	if (found) {
		found.innerHTML = totalItems.toString()	;	// Sostituisco il contenuto
	}
}
...
function displayBook(book: Book) {
	...
	div.setAttribute('class', 'col'); // -> <div class="col"></div>
	...
	if (searchEle) {
		...
		fromEvent(searchEle, 'keyup')
			.pipe(
				...
				debounceTime(1000),	// Impostiamo un delay di 1s per permettere all'utente di scrivere
				tap(() => cleanBooks()),
